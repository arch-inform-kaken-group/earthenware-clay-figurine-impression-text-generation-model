...
#hyp_lrnrate = {'name':'learning_rate', 'params':[1e-4, 1e-5]}
#hyp_lrnrate = {'name':'learning_rate', 'params':[1e-5, 1e-6]}
hyp_lrnrate = {'name':'learning_rate', 'params':[1e-6]}
...
###########################################################################
def train_test_run(trial_id, grid_params):
...
for i, btcsize in enumerate(hyp_btcsize['params']):
	for j, optmzr in enumerate(hyp_optm['params']):
		for k, lrnrt in enumerate(hyp_lrnrate['params']):
			...
			train_test_run(trial_id, grid_params)
			trial_id = trial_id + 1
